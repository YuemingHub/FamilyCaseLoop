...
def _strong_hits(text: str, phrases: List[str]) -> List[str]:
    return [p for p in phrases if p in text]


def _weak_hits(text: str, weak: List[str], need_intensity: bool) -> List[str]:
    """弱词需带强度词(动太多) 或 强度/害怕上下文(动太少) 才计分。"""
    ...
    if need_intensity:
        ...
        ok = has_intensity or has_fear
    return [w for w in weak if w in text] if ok else []
...
def diagnose_heuristic(text: str) -> Diagnosis:
    ...
    for cap, phrases in CAPACITY_STRONG.items():
        cap_scores[cap] = len(_strong_hits(text, phrases))
    # 取命中最多；同分按优先级
    ...
    if capacity != "维持" and cap_scores[capacity] > 0:
        evidence["承载力"] = _strong_hits(text, CAPACITY_STRONG[capacity])
    if tags:
        evidence["标签"] = tags
    if risk:
